...
public class ScheduleServiceImpl implements ScheduleService, ScheduleServiceForBl {
    ...
    private static final String VIEW_CONFLICT_ERROR_MESSAGE = "观众已可见，七天内无法临时修改";
    ...
    boolean isAudienceCanView(List<Integer> scheduleIdList) throws ParseException {
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd");
        Date today = simpleDateFormat.parse(simpleDateFormat.format(new Date()));
        // 由于数据库中 view 表不存在，并且前端和用户查询统一为7天，这里将从数据库查询改为直接写死 7
        Date endDate = getNumDayAfterDate(today, 7);
